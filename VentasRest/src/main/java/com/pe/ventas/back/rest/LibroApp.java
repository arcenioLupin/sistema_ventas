...
public class LibroApp implements SparkApplication {

    private static final Logger LOG = LogManager.getLogger(LibroApp.class);

    private final AbstractApplicationContext context = new ClassPathXmlApplicationContext("app-config.xml");

    @Override
    public void init() {
        final ILibroRest libroRest = (ILibroRest) context.getBean("libroRest");
